...
logLik_joint_city <- function(params, data) {
  # Paramètres
  gamma <- params[1:11] # Coefficients pour FP_star
  beta <- params[12:(19 + length(unique(data$CITY)))] # Coefficients pour la fonction de coût
  sigma <- params[20 + length(unique(data$CITY))] # Écart-type de epsilon
  rho <- params[21 + length(unique(data$CITY))] # Corrélation entre epsilon et eta
...
init_params_city <- c(rep(0.01, 19 + n_cities), 0.5, 0.05)
init_params_city <- c(rep(0.01, 19 + n_cities), 0.5, 0.05)

# Définir les contraintes linéaires
A_city <- matrix(c(
  rep(0, 19), rep(0, n_cities), 1, 0, # sigma - 0.01 >= 0
  rep(0, 19), rep(0, n_cities), 0, 1, # rho + 0.99 >= 0
  rep(0, 19), rep(0, n_cities), 0, -1 # 0.99 - rho >= 0
), ncol = 19 + n_cities + 2, byrow = TRUE)
