...
logLik_joint_refined <- function(params, data) {
  ...
  beta <- params[7:14]  # Coefficients for the cost function
...
joint_density <- dmvnorm(
    x = cbind(residuals, FP_star),
    sigma = matrix(c(sigma^2, rho * sigma, rho * sigma, sigma_eta^2), nrow = 2))
...
init_params <- c(rep(0.01, 14), 0.5, 0.05)  # 6 (gamma) + 8 (beta) + sigma + rho
