...
logLik_joint_city <- function(params, data) {
  ...
  beta <- params[7:(13 + length(unique(data$CITY)))]  # Coefficients pour la fonction de coût
  sigma <- params[14 + length(unique(data$CITY))]     # Écart-type de epsilon
  rho <- params[15 + length(unique(data$CITY))]       # Corrélation entre epsilon et eta

  # Paramètres dérivés
  sigma_eta <- 1  # Variance de eta fixée à 1
  # Générer les dummies pour les villes
  city_dummies <- model.matrix(~ CITY - 1, data = data)
  city_params <- beta[8:(7 + ncol(city_dummies))]
...
init_params_city <- c(rep(0.01, 13 + n_cities), 0.5, 0.05)

# Définir les contraintes linéaires
A_city <- matrix(c(
  rep(0, 13), rep(0, n_cities), 1, 0,  # sigma - 0.01 >= 0
  rep(0, 13), rep(0, n_cities), 0, 1,  # rho + 0.99 >= 0
  rep(0, 13), rep(0, n_cities), 0, -1 # 0.99 - rho >= 0
), ncol = 15 + n_cities, byrow = TRUE)

B_city <- c(-0.01, 0.99, 0.99)

# Maximisation de la log-vraisemblance avec contraintes linéaires et méthode Nelder-Mead
result_city <- maxLik(logLik = logLik_joint_city, start = init_params_city, data = data,
                      constraints = list(ineqA = A_city, ineqB = B_city), method = "Nelder-Mead")
